...
**Chi tiết đầy đủ đã chuyển sang `pharma-document/rag-reranking-guide.md`** — tài liệu đó gộp Phase 5 (threshold/grading/rewrite/escalation/observability) cùng với **reranking** (chưa từng có trong plan này) vào chung 1 pipeline, vì hai mảng ảnh hưởng lẫn nhau: rerank giúp threshold/grading dựa trên tín hiệu score sạch hơn thay vì cosine score thô. Xem file đó để có types/interfaces/skeleton/thứ tự làm/verify đầy đủ.
